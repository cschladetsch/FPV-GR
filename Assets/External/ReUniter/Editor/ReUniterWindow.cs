...
namespace Reuniter
{
    ...
    public class ReUniterWindow : EditorWindow
    {
        ...
        private static IEnumerable<ReUniterItemInfo> FindGameObjectsAction(string[] searchTerms, IEnumerable<Type> unityTypes)
        {
            ...
            return applicableTypes.SelectMany(x => FindObjectsOfType(x)).Select(x => new ReUniterItemInfo(BuildParentPath(x), x.name, x));
        }
        ...
        {
            var fileName = FileNameWithoutExtension(item.Name);
            ...
            itemInfos._Each((x, index) =>
                {
                    ...
                    GUILayout.Label(Highlight(x.Name, itemName) + AssetTypeSuffix(x), RichTextGuiStyle);
...
//            te.MoveCursorToPosition(new Vector2(5555, 5555));
        ...
        private string Highlight(string item, string searchTerms)
        {
